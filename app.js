(() => {
  ...
  function bindNativeAppleBridge() {
    ...
    [els.earnedAccessLabelInput, els.earnedAccessTaskMinutesInput, els.earnedAccessDailyLimitInput, els.earnedAccessAutomaticStepsInput, els.earnedAccessAutomaticUseInput, els.earnedAccessStepsInput, els.earnedAccessMinutesInput, els.earnedAccessModeInput].forEach(input => input.addEventListener('change', () => {
    ...
    window.addEventListener('dailyRoutine:native', event => {
      ...
      if (detail.name === 'native.ready') {
        ...
        earnedAccessNativeState = {
          ...earnedAccessNativeState,
          available: Boolean(value.earnedAccessAvailable),
          protectionEnabled: Boolean(value.earnedAccessProtectionEnabled),
          shielding: Boolean(value.earnedAccessShielding)
        };
        els.earnedAccessNativeStatus.textContent = value.earnedAccessAvailable ? 'Screen Time controls are ready.' : 'Screen Time controls are unavailable on this device.';
        syncWatchContext();
      ...
      } else if (detail.name === 'earned.access.status') {
        const value = detail.value || {};
        earnedAccessNativeState = {
          available: true,
  ...
  function earnedAccessDeviceSettings() {
    const defaults = {
      label: 'Selected apps', automaticSteps: true, automaticAccess: true, stepGoal: 8000, rewardMinutes: 60, taskRewardMinutes: 5, dailyLimitMinutes: 60, mode: 'fixed',
      ...
      bankByDate: {}, earnedByDate: {}, movementProgressByDate: {}, movementCreditedByDate: {}, taskCreditsByDate: {}, redemptionsByDate: {}
    };
    try {
      ...
      let needsSave = false;
      const needsAutomaticMigration = !parsed || typeof parsed !== 'object' || !Object.hasOwn(parsed, 'automaticSteps');
      const needsAutomaticAccessMigration = !parsed || typeof parsed !== 'object' || !Object.hasOwn(parsed, 'automaticAccess');
      if (needsAutomaticMigration) {
        next.automaticSteps = true;
        next.stepGoal = 8000;
        next.rewardMinutes = 60;
        next.active = null;
      } else {
        next.automaticSteps = Boolean(next.automaticSteps);
      }
      next.automaticAccess = needsAutomaticAccessMigration ? true : Boolean(next.automaticAccess);
      next.label = String(next.label || defaults.label).trim().slice(0, 80) || defaults.label;
      next.stepGoal = Math.min(30000, Math.max(1000, Math.round(Number(next.stepGoal) || defaults.stepGoal)));
      ...
      next.movementProgressByDate = next.movementProgressByDate && typeof next.movementProgressByDate === 'object' ? next.movementProgressByDate : {};
      next.movementCreditedByDate = next.movementCreditedByDate && typeof next.movementCreditedByDate === 'object' ? next.movementCreditedByDate : {};
      ...
      if (next.active?.dateKey && next.active.dateKey !== dateKey(startOfToday())) {
        next.active = null;
        needsSave = true;
      }
      if (!next.activeAllowance || typeof next.activeAllowance !== 'object') {
        next.activeAllowance = null;
      } else {
        const startedAt = new Date(next.activeAllowance.startedAt || '');
        const allowanceDay = String(next.activeAllowance.dateKey || (!Number.isNaN(startedAt.getTime()) ? dateKey(startedAt) : ''));
        if (!allowanceDay || allowanceDay !== dateKey(startOfToday())) {
          next.activeAllowance = null;
          needsSave = true;
        } else if (next.activeAllowance.dateKey !== allowanceDay) {
          next.activeAllowance = { ...next.activeAllowance, dateKey: allowanceDay };
          needsSave = true;
        }
      }
      next.earnedUntil = '';
      if (!next.lastCompleted || typeof next.lastCompleted !== 'object') next.lastCompleted = null;
      if (needsAutomaticMigration || needsAutomaticAccessMigration || needsSave) localStorage.setItem(EARNED_ACCESS_DEVICE_KEY, JSON.stringify(next));
      return next;
    } catch { return defaults; }
  }

  function saveEarnedAccessDeviceSettings(updates) {
    const next = { ...earnedAccessDeviceSettings(), ...updates };
    localStorage.setItem(EARNED_ACCESS_DEVICE_KEY, JSON.stringify(next));
    return next;
  }
  ...
  function saveEarnedAccessFormSettings() {
    return saveEarnedAccessDeviceSettings({
      ...
      automaticSteps: Boolean(els.earnedAccessAutomaticStepsInput.checked),
      automaticAccess: Boolean(els.earnedAccessAutomaticUseInput.checked),
      ...
      stepGoal: Math.min(30000, Math.max(1000, Math.round(Number(els.earnedAccessStepsInput.value) || 8000))),
      rewardMinutes: Math.min(120, Math.max(15, Math.round(Number(els.earnedAccessMinutesInput.value) || 60))),
  ...
  function earnedAccessRequirement(settings = earnedAccessDeviceSettings()) {
    if (settings.mode !== 'escalating') return settings.stepGoal;
    const completedToday = Math.max(0, Math.round(Number(settings.roundsByDate[dateKey(startOfToday())]) || 0));
    return [750, 1000, 1500][Math.min(completedToday, 2)];
  }

  function latestHealthStepsToday() {
    if (!latestHealthSummary?.date) return null;
  ...
  function applyAutomaticMovementRewards() {
    const settings = earnedAccessDeviceSettings();
    if (!settings.automaticSteps) return settings;
    const steps = latestHealthStepsToday();
    if (steps === null) return settings;
    const today = dateKey(startOfToday());
    const goal = Math.max(1000, Number(settings.stepGoal) || 8000);
    const maximum = Math.min(settings.dailyLimitMinutes, Math.max(15, Number(settings.rewardMinutes) || 60));
    const progressMinutes = Math.min(maximum, Math.floor((Math.min(steps, goal) / goal) * maximum));
    const previousProgress = Math.max(0, Number(settings.movementProgressByDate[today]) || 0);
    if (progressMinutes <= previousProgress) return settings;
    const tracked = saveEarnedAccessDeviceSettings({
      active: null,
      movementProgressByDate: { ...settings.movementProgressByDate, [today]: progressMinutes }
    });
    const award = addMinutesToEarnedAccessBank(tracked, progressMinutes - previousProgress, 'walking');
    const credited = Math.max(0, Number(settings.movementCreditedByDate[today]) || 0) + award.minutes;
    const next = saveEarnedAccessDeviceSettings({
      movementCreditedByDate: { ...award.settings.movementCreditedByDate, [today]: credited }
    });
    if (award.minutes) showToast(`${award.minutes} walking minute${award.minutes === 1 ? '' : 's'} added automatically.`);
    return next;
  }

  function automaticallyOpenEarnedAccess(settings) {
  ...
  function redeemEarnedAccessBank() {
    ...
    const redemptions = Array.isArray(settings.redemptionsByDate[today]) ? settings.redemptionsByDate[today] : [];
    const next = saveEarnedAccessDeviceSettings({
      activeAllowance: { id: redemptionID, minutes: rewardMinutes, startedAt: startedAt.toISOString(), dateKey: today },
  ...
  function startEarnedAccessRound() {
    ...
    const active = {
      ...
      baselineSteps: steps,
      requiredSteps: earnedAccessRequirement(settings),
  ...
  function cancelEarnedAccessRound() {
    ...
    renderEarnedAccess();
    showToast('Earned Access round canceled.');
  }

  function updateEarnedAccessFromHealth() {
    if (pendingEarnedAccessStart) {
      pendingEarnedAccessStart = false;
      startEarnedAccessRound();
      return;
    }
    const settings = earnedAccessDeviceSettings();
    const active = settings.active;
    const steps = latestHealthStepsToday();
    if (!active || steps === null || active.dateKey !== dateKey(startOfToday())) {
      renderEarnedAccess();
      return;
    }
  ...
  function renderEarnedAccess() {
    ...
    settings = automaticallyOpenEarnedAccess(settings);
    const today = dateKey(startOfToday());
    ...
    els.earnedAccessAutomaticStepsInput.checked = settings.automaticSteps;
    els.earnedAccessAutomaticUseInput.checked = settings.automaticAccess;
    ...
    const active = settings.active;
    const foundationLocked = document.body.classList.contains('truth-locked');
    const accessIsEarned = Boolean(settings.activeAllowance) && !foundationLocked;
    ...
    const nativeAllowanceMatches = Boolean(settings.activeAllowance)
      && earnedAccessNativeState.allowanceActive
      && earnedAccessNativeState.allowanceRedemptionID === settings.activeAllowance?.id;
    const allowanceRemaining = settings.activeAllowance
      ? nativeAllowanceMatches && Number.isFinite(earnedAccessNativeState.allowanceRemainingMinutes)
        ? Math.max(0, earnedAccessNativeState.allowanceRemainingMinutes)
        : Math.max(0, Number(settings.activeAllowance.minutes) || 0)
      : 0;
    const totalAvailable = Math.min(settings.dailyLimitMinutes, bankMinutes + allowanceRemaining);
    const stepsToday = latestHealthStepsToday() ?? Math.max(0, Number(state.days[today]?.entries?.['apple-health-steps']) || 0);
    const stepGoal = Math.max(1000, Number(settings.stepGoal) || 8000);
    const stepPercent = Math.min(100, Math.round((stepsToday / stepGoal) * 100));
    ...
    let percent = 0;
    if (settings.automaticSteps && !accessIsEarned) {
      const steps = stepsToday;
      const goal = stepGoal;
      const maximum = Math.min(settings.dailyLimitMinutes, Math.max(15, Number(settings.rewardMinutes) || 60));
      const credited = Math.max(0, Number(settings.movementCreditedByDate[today]) || 0);
      percent = Math.min(100, Math.round((steps / goal) * 100));
      els.earnedAccessBadge.textContent = steps >= goal ? 'Goal reached' : 'Automatic';
      els.earnedAccessStatus.textContent = `${Math.min(steps, goal).toLocaleString()} of ${goal.toLocaleString()} steps`;
      els.earnedAccessDetail.textContent = `${credited} walking minute${credited === 1 ? '' : 's'} added today. Reaching ${goal.toLocaleString()} steps can add up to ${maximum} minutes, within the overall daily limit.`;
    } else if (pendingEarnedAccessStart) {
    ...
    const gate = els.earnedAccessGateStatus.closest('.earned-access-gate');
    gate.classList.remove('is-locked', 'is-open', 'is-warning', 'is-ready');
    if (!earnedAccessNativeState.available) {
      gate.classList.add('is-warning');
      els.earnedAccessGateStatus.textContent = 'Finish setup in the iPhone app';
      els.earnedAccessGateDetail.textContent = 'Allow Screen Time, choose earned apps, and turn on the morning gate.';
      els.earnedAccessBadge.textContent = 'Setup needed';
    } else if (!earnedAccessNativeState.morningGateEnabled) {
      gate.classList.add('is-warning');
      ...
      els.earnedAccessGateDetail.textContent = 'Yesterday’s unused minutes cannot open apps today.';
      els.earnedAccessBadge.textContent = 'Locked';
    } else if (!earnedAccessNativeState.protectionEnabled) {
      gate.classList.add('is-warning');
      els.earnedAccessGateStatus.textContent = 'Earned-app protection is off';
      els.earnedAccessGateDetail.textContent = 'The morning gate may be complete, but the selected earned apps are not being protected.';
      els.earnedAccessBadge.textContent = 'Setup needed';
    } else if (accessIsEarned) {
      gate.classList.add('is-open');
      els.earnedAccessGateStatus.textContent = `${totalAvailable} earned minute${totalAvailable === 1 ? '' : 's'} available`;
      els.earnedAccessGateDetail.textContent = 'Only foreground time in the selected earned apps reduces this shared balance.';
      els.earnedAccessBadge.textContent = 'Open';
    } else if (earnedAccessNativeState.shielding) {
      gate.classList.add('is-locked');
      els.earnedAccessGateStatus.textContent = 'Earned apps are locked';
      els.earnedAccessGateDetail.textContent = 'Complete a selected routine or keep walking to add time.';
      els.earnedAccessBadge.textContent = 'Locked';
    } else {
      gate.classList.add('is-ready');
      els.earnedAccessGateStatus.textContent = 'Protection is ready';
  ...
  function syncNativeStepRewardAutomation(settings) {
    ...
    const enabled = Boolean(settings.automaticSteps && healthDeviceSettings().connected);
    const goalSteps = Math.max(1000, Number(settings.stepGoal) || 8000);
    const maxMinutes = Math.min(settings.dailyLimitMinutes, Math.max(15, Number(settings.rewardMinutes) || 60));
    const directive = `${enabled}:${goalSteps}:${maxMinutes}`;
    if (directive === lastStepRewardNativeDirective) return;
    lastStepRewardNativeDirective = directive;
    sendNativeBridgeMessage('health.step-rewards.configure', { enabled, goalSteps, maxMinutes });
  }

  function syncNativeEarnedAccess(settings, accessIsEarned) {
    if (!window.DailyRoutineNative?.postMessage) return;
    const foundationLocked = document.body.classList.contains('truth-locked');
    const directive = !foundationLocked && accessIsEarned && settings.activeAllowance?.id ? `allow:${settings.activeAllowance.id}` : 'lock';
    if (directive === lastEarnedAccessNativeDirective) return;
    lastEarnedAccessNativeDirective = directive;
    if (!foundationLocked && accessIsEarned) sendNativeBridgeMessage('earned.access.allow', { minutes: settings.activeAllowance.minutes, redemptionId: settings.activeAllowance.id, label: settings.label });
  ...
  function openNoteDialog(note = null, prefill = {}) {
    ...
    if (!window.matchMedia('(max-width: 560px)').matches) setTimeout(() => els.noteTextInput.focus(), 0);
  }
