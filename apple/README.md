...
Build 16 adds the privacy-minimized iPhone Home Screen widget and a simpler native Setup flow. The large widget uses a balanced command-center layout with explicit Truth Before Tasks and Convictions status, progress, safe routine actions, freshness time, and an aggregate Earned Access balance that never includes selected app identities. Widget setup, Health metrics, Earned Access, and the Apple Watch companion now have separate focused cards while retaining the existing on-device behavior.
