...
Build 14 begins the Family Controls development test. It adds individual Screen Time authorization, Apple's privacy-preserving app and website picker, on-device selection persistence, and explicit Managed Settings controls to apply and remove a real shield. The first physical-device test intentionally keeps this shield separate from routine completion and reward timers. Automatic timed relocking will require a Device Activity extension, and TestFlight distribution requires Apple's Family Controls distribution entitlement for every participating target.
